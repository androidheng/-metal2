...
public class DataServiceImpl implements DataService {
	...
	public PageResult findPage(TbData data, int pageNum, int pageSize) {
		...
		@Override
		public PageResult findYun(String type, String date,Integer cid, int pageNum, int pageSize) {
			PageHelper.startPage(pageNum, pageSize);
			if(type.equals("0")) {
				Page<TbData> page= (Page<TbData>)dataMapper.findYunDay(date,cid);		
				return new PageResult(0,"",page.getTotal(), page.getResult());
			}else if(type.equals("1")) {
				Page<TbData> page= (Page<TbData>)dataMapper.findYunMonth(date,cid);		
				return new PageResult(0,"",page.getTotal(), page.getResult());
			}else {
				Page<TbData> page= (Page<TbData>)dataMapper.findYunYear(date,cid);		
				return new PageResult(0,"",page.getTotal(), page.getResult());
			}
		...
		public PageResult statistical(String type, Integer mid, Integer wid, String date, int pageNum, int pageSize) {
			...
			if(type.equals("0")) {
				...
				Page<TbData> page= (Page<TbData>)dataMapper.findMonth(date.substring(0, 7), mid, wid);		
				return new PageResult(0,"",page.getTotal(), page.getResult());
			}else {
				Page<TbData> page= (Page<TbData>)dataMapper.findYear(date.substring(0, 4), mid, wid);		
				return new PageResult(0,"",page.getTotal(), page.getResult());
			}
		}
